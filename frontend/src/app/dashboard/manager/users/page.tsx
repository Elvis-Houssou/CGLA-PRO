...
export default function MyUsersPage() {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            ...
            setLoading(true);
            try {
                const response = await User.getManagerUsers();
                if (response.status === 200) {
                    setUsers(response.data.all_users);
                    setOriginalUsers(response.data.all_users); // Sauvegarde des données originales
                } else {
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {users
                                        ...
                                        .map((userData, index) => {
                                            console.log("User role:", userData.firstname);
                                            return (
