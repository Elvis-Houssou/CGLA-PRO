...
def upgrade() -> None:
    ...
    # 1. Supprimer la valeur par défaut de la colonne role
    ...
    # 4. Remettre la valeur par défaut avec le nouveau type
    op.execute("ALTER TABLE \"user\" ALTER COLUMN role SET DEFAULT 'station_owner'::roleuser;")
...
def downgrade() -> None:
    ...
    # 2. Recréer l'ancien type role
    op.execute("""
        CREATE TYPE role AS ENUM (
            'super_admin', 'system_manager', 'station_owner',
            'station_manager', 'car_washer', 'station_client'
        );
    """)
    ...
    # 4. Remettre le défaut
    op.execute("ALTER TABLE \"user\" ALTER COLUMN role SET DEFAULT 'station_owner'::role;")

    # 5. Supprimer roleuser
    op.execute("DROP TYPE roleuser;")
    # ### commands auto generated by Alembic - please adjust! ###
    op.alter_column('user', 'role',
